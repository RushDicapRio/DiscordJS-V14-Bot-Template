...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isUserContextMenuCommand()) { // Utilisateur :
  ...
  if (interaction.isModalSubmit()) { // Modaux :
    const modal = client.modals.get(interaction.customId);

    if (!modal) return interaction.reply({
      embeds: [
        new EmbedBuilder()
          .setDescription("Quelque chose s'est mal passé... L'ID modal n'est probablement pas défini dans le gestionnaire de modaux.")
          .setColor('Red')
      ],
      ephemeral: true
    });
